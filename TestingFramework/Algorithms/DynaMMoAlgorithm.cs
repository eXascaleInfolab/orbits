...
namespace TestingFramework.Algorithms
{
    public partial class DynaMMoAlgorithm : Algorithm
    {
        ...
        private Process GetNnmfProcess(DataDescription data, int len, string rtVal, string rtStrVal)
        {
            ...
            nnmfproc.StartInfo.Arguments = $"-alg dynammo -test {testType} -n {data.N} -m {data.M} -k {AlgoPack.TypicalTruncation} " +
        ...
        private void RunNnmf(Process nnmfproc)
        {
            ...
            {
                string errText =
                    $"[WARNING] DynaMMo returned code {nnmfproc.ExitCode} on exit.{Environment.NewLine}" +
