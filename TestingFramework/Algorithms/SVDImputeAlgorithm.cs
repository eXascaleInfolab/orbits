...
namespace TestingFramework.Algorithms
{
    public partial class SVDImputeAlgorithm : Algorithm
    {
        ...
        private Process GetNnmfProcess(DataDescription data, int len, string rtVal, string rtStrVal)
        {
            ...
            nnmfproc.StartInfo.Arguments = $"-alg itersvd -test {testType} -n {data.N} -m {data.M} -k {AlgoPack.TypicalTruncation} " +
        ...
        private void RunNnmf(Process nnmfproc)
        {
            ...
            {
                string errText =
                    $"[WARNING] SVDImpute returned code {nnmfproc.ExitCode} on exit.{Environment.NewLine}" +
